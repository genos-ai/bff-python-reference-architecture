...
All logs are written to a single JSONL file: `var/logs/system.jsonl`. Filter by the `source` field to isolate logs from a specific origin.
...
```bash
# View recent errors
jq 'select(.level == "error")' var/logs/system.jsonl | tail -20

# Find all logs for a specific request
jq 'select(.request_id == "abc-123")' var/logs/system.jsonl

# Filter by source
jq 'select(.source == "telegram")' var/logs/system.jsonl

# Find all resilience events (circuit breakers, retries, timeouts)
jq 'select(.resilience_event != null)' var/logs/system.jsonl

# Correlate logs with a specific trace
jq 'select(.trace_id == "4bf92f3577b34da6a3ce929d0e0e4736")' var/logs/system.jsonl

# Watch logs in real-time
tail -f var/logs/system.jsonl | jq .

# Count errors by source
jq 'select(.level == "error") | .source' var/logs/system.jsonl | sort | uniq -c | sort -rn
```
