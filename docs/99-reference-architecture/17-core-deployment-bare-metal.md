...
The application code is identical regardless of whether it deploys to bare metal or Azure (18). This document covers only the infrastructure: server setup, service configuration, release management, backup procedures, and monitoring integration. It implements the security standards (13) at the infrastructure level and integrates with background tasks (15) for worker and scheduler service definitions.
...
- Install pyenv
- Install **Python 3.14** (see 04-core-backend-architecture.md and 16-core-concurrency-and-resilience.md for version requirements)
...
`uvloop` is required per 04-core-backend-architecture.md. `tenacity` and `aiobreaker` are required per 16-core-concurrency-and-resilience.md. `py-spy` is required per 08-core-observability.md for production debugging.
...
|---------|-------|-----------|
| `--loop uvloop` | uvloop event loop | 2–4x faster than default (doc 16) |
| `--timeout-graceful-shutdown 30` | 30 seconds to drain | Matches shutdown sequence in doc 16 |
...
Only one scheduler instance should run (see doc 15):
...
All services handle shutdown signals gracefully per **16-core-concurrency-and-resilience.md**:
...
For the full observability standard — three pillars (logs, metrics, traces), profiling tools, and resilience event logging — see **08-core-observability.md**.

### Log Collection

Logs written to `/opt/{app-name}/shared/var/logs/`:
...
See **08-core-observability.md** Profiling section for the full diagnostic workflow.
...
```
        ...
        │ endpoint │ │(var/logs)│ │ Collector  │
        └──────────┘ └──────────┘ └────────────┘
```

OpenTelemetry is configured in the application via `config/settings/observability.yaml` (see doc 08). The infrastructure components above are deployed as systemd services alongside the application.
...
|----------|-------------|
| 04-core-backend-architecture.md | Python 3.14, uvloop, FastAPI configuration |
| 08-core-observability.md | Production stack (Prometheus, Loki, Tempo, Grafana), profiling tools, health check specification |
| 13-core-security-standards.md | Server hardening, TLS, firewall configuration |
| 15-core-background-tasks.md | Worker and scheduler systemd service definitions |
| 18-core-deployment-azure.md | Alternative deployment target — same application code |
| 16-core-concurrency-and-resilience.md | Graceful shutdown sequence, uvloop, file descriptor limits |
