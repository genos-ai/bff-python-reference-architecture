# 15 — Background Tasks and Scheduling
...
- 2.0.0 (2026-03-01): Added context propagation (request_id, correlation_id, trace_id) for background tasks. Added resilience patterns for tasks calling external services. Added task observability with structured logging. References 08-core-observability.md v3 and 16-core-concurrency-and-resilience.md.
- 1.0.0 (2026-02-11): Extracted from 04-core-backend-architecture.md into dedicated document
...
The single-scheduler-instance rule (only one scheduler process runs, ever) prevents the duplicate execution problem that plagues cron-based systems. This document was extracted from backend architecture (04) into its own standard because background task patterns touch deployment (17, 18), observability (08), and integration modules like Telegram (24) — enough cross-cutting concerns to justify dedicated guidance.
...
For OpenTelemetry trace propagation across the process boundary, see **08-core-observability.md** Context Propagation section.
...
Background tasks that call external services must use the resilience stack from **16-core-concurrency-and-resilience.md**: circuit breaker → retry → timeout.
...
All resilience events (retry attempts, circuit breaker state changes, timeouts) are logged per the contract in **08-core-observability.md**.
...
1. Check worker logs for error details — filter by `correlation_id` or `request_id`:
   ```bash
   jq 'select(.source == "tasks" and .level == "error")' var/logs/system.jsonl | tail -20
   ```
2. Check for resilience events — circuit breaker open, retries exhausted:
   ```bash
   jq 'select(.resilience_event != null and .source == "tasks")' var/logs/system.jsonl
   ```
...
|----------|-------------|
| 04-core-backend-architecture.md | Task dispatching from API endpoints |
| 21-opt-event-architecture.md | Outbox relay and consumer lag collection run as scheduled tasks |
| 08-core-observability.md | Context propagation contract, resilience event logging, structured logging |
| 17-core-deployment-bare-metal.md | Worker and scheduler systemd service definitions |
| 18-core-deployment-azure.md | Worker deployment on WebJobs or separate App Service |
| 16-core-concurrency-and-resilience.md | Resilience patterns (circuit breaker, retry, timeout) for tasks calling external services |
