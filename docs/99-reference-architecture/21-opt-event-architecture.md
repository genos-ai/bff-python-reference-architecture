...
The transactional outbox pattern is unchanged — it remains the correct solution for the dual-write problem. This module provides the communication backbone for inter-module events (05), real-time client updates (22, 27), and is a required dependency for the agentic architecture (31).
...
Rationale:
- Already deployed for caching and task queues (doc 15)
...
Schedule this as a Taskiq periodic task (doc 15) running every 5 minutes for each consumer group.
...
_notification_breaker = aiobreaker.CircuitBreaker(
    fail_max=5,
    timeout_duration=30,
    listeners=[ResilienceLogger("notification_service")],  # From doc 08
)
...
| trace_id | OpenTelemetry trace ID for distributed trace correlation (see doc 08). Null if tracing disabled. |
| payload | Event-specific data |

### Trace Context in Events

When OpenTelemetry tracing is enabled (doc 08), include `trace_id` in the event envelope. This links the event to the distributed trace that produced it, enabling end-to-end visibility from HTTP request → event publish → consumer processing:
