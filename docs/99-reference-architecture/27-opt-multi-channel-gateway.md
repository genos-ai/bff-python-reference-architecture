# 27 — Multi-Channel Gateway (Optional Module)
...
**Dependencies**: This module requires **04-core-backend-architecture.md** (FastAPI application) and **24-opt-telegram-bot-integration.md** (reference channel adapter).
...
|--------|------|-----------|
| **24-opt-telegram-bot-integration.md** | First channel adapter (Telegram) | ✅ Becomes a channel adapter under this module's interface |
| **26-opt-tui-architecture.md** | TUI client | ✅ Connects via WebSocket defined here |
| **22-opt-frontend-architecture.md** | React web frontend | ✅ Connects via REST + WebSocket defined here |
...
The existing architecture defines several client types — React web frontend (22), CLI (22), Telegram bot (24), TUI (26) — each connecting to the FastAPI backend independently. Each has its own connection model, authentication mechanism, and message format. There is no shared session layer across these clients, no centralized security enforcement point, and no mechanism for the backend to push events to clients that are not actively polling.
...
```
    ...
    │  - deliver_response() ← ChannelResponse                  │
    ...
    │  - Agent requests → Coordinator (doc 32)               │
...
6. Coordinator processes via agent runtime (doc 32)
7. Response returned to router

**Outbound** (agent response delivered to user):
1. Router receives `ChannelResponse` from coordinator
...
class ChannelAdapter(ABC):
    ...
    @abstractmethod
    async def deliver_response(self, response: ChannelResponse) -> bool:
        """
...
Adapters handle these differences internally. The router and session manager work exclusively with `ChannelMessage` and `ChannelResponse` — they never see channel-native formats.
...
Tool access is configured per session in `config/settings/gateway.yaml` and enforced by the session manager before the coordinator receives the request. The coordinator's `CoordinatorRequest` (doc 32) carries the session's `tool_access_level`, and the tool registry filters available tools accordingly.
...
The FastAPI application exposes a WebSocket endpoint for real-time bidirectional communication. This serves the TUI (doc 26), the React frontend (22), and any other client that needs server-pushed events.
...
Events pushed through WebSocket follow the event schema from **21-opt-event-architecture.md** with gateway-specific additions:
...
2. Server validates the token using the existing JWT infrastructure (06-core-authentication.md)
...
1. Router constructs a `CoordinatorRequest` (doc 32) from the `ChannelMessage`
2. `session_id`, `user_id`, `entry_point`, and `tool_access_level` are populated from the session
3. Router calls `handle(request)` (doc 32)
4. Response is wrapped in an `ChannelResponse` and delivered through the channel adapter
...
│   ├── __init__.py
│   ├── adapters/
│   │   ├── __init__.py              # ChannelAdapter base class, ChannelMessage, ChannelResponse
...
The existing `modules/telegram/` module is not replaced. The Telegram channel adapter (`modules/gateway/adapters/telegram.py`) wraps it — translating between the gateway's `ChannelMessage`/`ChannelResponse` format and aiogram's native types. The Telegram module retains its handlers, keyboards, FSM states, and middlewares. The gateway adds the session management, cross-channel routing, and security enforcement layer on top.
...
```python
from modules.gateway.adapters import ChannelAdapter, ChannelMessage, ChannelResponse
...
from modules.gateway.adapters import ChannelResponse
...
- [ ] Implement `ChannelAdapter` base class and data models (`ChannelMessage`, `ChannelResponse`)
...
- [04-core-backend-architecture.md](04-core-backend-architecture.md) — FastAPI application where WebSocket endpoint is mounted
- [24-opt-telegram-bot-integration.md](24-opt-telegram-bot-integration.md) — Telegram module (first channel adapter)
- [26-opt-tui-architecture.md](26-opt-tui-architecture.md) — TUI client (connects via WebSocket defined here)
- [22-opt-frontend-architecture.md](22-opt-frontend-architecture.md) — React web frontend, CLI
- [21-opt-event-architecture.md](21-opt-event-architecture.md) — Event types, WebSocket patterns
- [06-core-authentication.md](06-core-authentication.md) — JWT authentication (extended for WebSocket)
- [08-core-observability.md](08-core-observability.md) — Source-based logging, X-Frontend-ID
